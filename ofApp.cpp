...
void ofApp::draw() {
	...
	for (int v = 0; v <= 360; v += v_span) {
		...
		bool flag = true;
		int u_start = ofMap(ofNoise(noise_seed, cos(v * DEG_TO_RAD) * 0.5, sin(v * DEG_TO_RAD) * 0.5, ofGetFrameNum() * 0.0025), 0, 1, -360 * 3, 360 * 3);
		int next_u = ofMap(ofNoise(noise_seed, cos((v + v_span) * DEG_TO_RAD) * 0.5, sin((v + v_span) * DEG_TO_RAD) * 0.5, ofGetFrameNum() * 0.0025), 0, 1, -360 * 3, 360 * 3);
		for (int u = u_start; u < u_start + 360; u += u_span) {

			if (flag) {

				face.addVertex(this->make_point(R, r, u, v));
				face.addVertex(this->make_point(R, r, u + u_span * 0.25, v));
				face.addVertex(this->make_point(R, r, next_u + u_span * 0.25, v + v_span));
				face.addVertex(this->make_point(R, r, next_u, v + v_span));

				line.addVertex(this->make_point(R, r, u, v));
				line.addVertex(this->make_point(R, r, u + u_span * 0.25, v));
				line.addVertex(this->make_point(R, r, next_u + u_span * 0.25, v + v_span));
